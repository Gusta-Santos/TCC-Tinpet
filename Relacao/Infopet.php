...
<body>
  ...
  <?php
    ...
    <script>
      Swal.fire({
        icon: 'error',
        title: 'Não foi possível registrar a relação',
        text: 'Por favor, tente novamente',
      })
    </script>
    <?php
    }
    if($_SESSION["errado"] == "iguais"){
    ?>
    <script>
      Swal.fire({
        icon: 'error',
        title: 'Essa relação já existe!',
        text: 'Vá para a página de menu de relações para encontra-la',
      })
    </script>
    <?php
    }
    ...
    if(isset($_POST["codigo"])){
      $cod = $_POST["codigo"];
      $_SESSION["Pet_codigo2"] = $cod;
    } else {
      $cod = $_SESSION["Pet_codigo2"];
    }
   ...
   <div id="interface">
    ...
    <br>
    <a href="../Relacao/Registrar_relacao.php"><button class="btn btn-dark btn-lg" style="background-color: rgb(82, 59, 26);">Tenho interesse</button></a>
    </div>
  </aside> 
